    ...
    class Server
      ...
      private
        ...
        def handle_put(path:, body:, storage:, resource_type: nil, **ctx)
          col_path = path.to_s
          if col_path.end_with?('/')
            col_path_slash = col_path
          else
            col_path_slash = "#{col_path}/"
          end
          ...
          if collection || col_path.end_with?('/')
            put_whole_collection(path: col_path_slash, body: body, storage: storage, resource_type: resource_type)
          else
        ...
        def extract_headers(env)
          {}.tap do |h|
            if env['HTTP_DEPTH']
              h['depth'] = env['HTTP_DEPTH']
            end
            if env['HTTP_IF_MATCH']
              h['if-match'] = env['HTTP_IF_MATCH']
            end
            if env['HTTP_IF_NONE_MATCH']
              h['if-none-match'] = env['HTTP_IF_NONE_MATCH']
            end
            if env['HTTP_DESTINATION']
              h['destination'] = env['HTTP_DESTINATION']
            end
            if env['HTTP_OVERWRITE']
              h['overwrite'] = env['HTTP_OVERWRITE']
            end
            if env['CONTENT_TYPE']
